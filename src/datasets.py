...
class MemesDataset(Dataset):
    def __init__(self, root_folder, dataset, split='train', image_size=224, fast=True):
        ...
        if self.fast:
            self.embds = torch.load(f'{self.root_folder}/{self.dataset}/clip_embds/{split}_no-proj_output.pt', map_location=torch.device('cuda:0'))
...
def load_dataset(args, split):                # !!! ACTION REQUIRED !!! - update path
    dataset = MemesDataset(root_folder='/YOUR_PATH/resources/datasets', dataset=args.dataset, split=split,
