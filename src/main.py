...
def main(args):
    ...
    if args.reproduce:
        assert args.pretrained_model.startswith(f'{args.dataset}'), 'The pretrained model and the dataset selected' \
                                                                   ' are not compatible'
        # load pretrained model     !!! ACTION REQUIRED !!! - update path
        model = HateClassifier.load_from_checkpoint(f'/YOUR_PATH/resources/pretrained_models/{args.pretrained_model}', args=args,
                                                    map_location=None)
    else:
        # create model
